"""Prioritized Replay Buffer + action masks
...
class PrioritizedMaskedReplayBuffer:
    ...
    def __init__(
        ...
        Args:
            ...
            num_actions: Number of env actions (to determine the shape of masks)
        ...
        self.obs_action_masks = np.zeros(
            (max_size, num_actions), dtype=action_dtype)
        self.next_obs = np.zeros((max_size,) + (obs_shape), dtype=obs_dtype)
        self.next_obs_action_masks = np.zeros(
            (max_size, num_actions), dtype=action_dtype)
    ...
    def add(self, obs, obs_action_mask, action, reward, next_obs, next_obs_action_mask, done, priority=None):
        ...
        Args:
            ...
            next_obs_action_mask: Action mask in the next observation
            done: Done
            priority: Priority of the new experience

        """
        self.obs[self.ptr] = np.array(obs).copy()
        self.obs_action_masks[self.ptr] = np.array(obs_action_mask).copy()
        self.next_obs[self.ptr] = np.array(next_obs).copy()
        self.next_obs_action_masks[self.ptr] = np.array(
            next_obs_action_mask).copy()
        self.actions[self.ptr] = np.array(action).copy()
        self.rewards[self.ptr] = np.array(reward).copy()
        self.dones[self.ptr] = np.array(done).copy()

        self.tree.set(
            self.ptr, self.min_priority if priority is None else priority)
    ...
    def sample(self, batch_size, to_tensor=False, device=None):
        ...
        experience_tuples = (
            self.obs[idxes],
            self.obs_action_masks[idxes],
            self.actions[idxes],
            self.rewards[idxes],
            self.next_obs[idxes],
            self.next_obs_action_masks[idxes],
            self.dones[idxes],
        )
        if to_tensor:
            # , weights)
    ...
    def get_all_data(self, max_samples=None, to_tensor=False, device=None):
        ...
        tuples = (
            self.obs[inds],
            self.obs_action_masks[inds],
            self.actions[inds],
            self.rewards[inds],
            self.next_obs[inds],
            self.next_obs_action_masks[inds],
            self.dones[inds],
        )
        if to_tensor:
