...
class VecEnvelope(MOPolicy, MOAgent):
    """Envelope Q-Leaning Algorithm, adapted for vectorized environments
    ...
    def __init__(
        ...
        if checkpoint_freq is not None:
            assert checkpoint_freq % env.num_envs == 0, "checkpoint_freq must be a multiple of the number of environments."
    ...
    def train(
        ...
        assert eval_freq % self.env.num_envs == 0, "eval_freq must be a multiple of n_envs."
        assert self.log and log_progress_every % self.env.num_envs == 0, "log_progress_every must be a multiple of n_envs."
        ...
        for _ in range(1, total_timesteps + 1, self.env.num_envs):
            ...
            for idx in range(self.env.num_envs):
                if vec_terminated[idx] or vec_truncated[idx]:
                    if vec_truncated[idx]:
                        iteration_num_invalid_episodes += 1  # In nxg, invalid episodes are indicated with truncated to True
                    episode_lens.append(episode_steps[idx])
                    episode_steps[idx] = 0

                    obs = next_vec_obs[idx]
                    num_episodes += 1
                    self.num_episodes += 1
                    #
